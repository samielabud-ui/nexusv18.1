    ...
    return (
      <div className="max-w-[1200px] mx-auto animate-in fade-in slide-in-from-bottom-8 duration-700 px-4 pt-10">
        <div className="bg-neutral-900/40 border border-blue-600/20 p-12 md:p-24 rounded-[3rem] text-center border-dashed backdrop-blur-sm">
          ...
          <h2 className="text-3xl md:text-5xl font-black text-white mb-6 tracking-tighter">Habilidades Profissionais Premium</h2>
    ...
    return (
      <div className="max-w-[1200px] mx-auto animate-in fade-in duration-500 px-4">
        ...
        <header className="mb-10">
          ...
          <h2 className="text-3xl font-black text-white tracking-tight">Conteúdos de HP</h2>
        </header>
        ...
        {selectedMed === 3 ? (
          <div className="space-y-4">
            {HP_MED3_LESSONS.map((lesson, idx) => (
              <div 
                key={idx} 
                className="bg-neutral-900/40 border border-neutral-800 p-6 rounded-2xl flex flex-col sm:flex-row justify-between items-center gap-4 hover:border-blue-500/30 transition-all group"
              >
                ...
                <button 
                  ...
                  className="w-full sm:w-auto px-6 py-3 rounded-xl bg-white text-black text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all text-center"
                >
                  Abrir Resumo
                </button>
              </div>
          ...
          <div className="bg-neutral-900/30 border border-neutral-800 p-12 md:p-20 rounded-[2.5rem] text-center border-dashed">
            ...
            <p className="text-neutral-500 text-sm max-w-md mx-auto leading-relaxed">
              Estamos organizando os checklists e resumos de habilidades profissionais para o <span className="text-white font-bold">Med {selectedMed}</span>.
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="max-w-[1600px] mx-auto animate-in fade-in duration-500 px-4">
      <header className="mb-12">
        <h2 className="text-4xl md:text-6xl font-black text-white mb-6 tracking-tighter italic">HP</h2>
      ...
      <section className="mb-16">
        <h3 className="text-[10px] font-black text-neutral-500 uppercase tracking-[0.4em] mb-8 flex items-center gap-4">
          Módulos HP <div className="h-px flex-grow bg-neutral-800/50"></div>
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-8 gap-4">
          {meds.map((m) => (
            <div 
              ...
              className="bg-neutral-900/30 border border-neutral-800 p-6 rounded-[2rem] cursor-pointer hover:bg-neutral-900 hover:border-blue-600/50 transition-all group flex flex-col items-center justify-center gap-3 shadow-xl"
            >
              ...
              <h4 className="text-xl font-black text-white">Med {m}</h4>
            </div>
          ))}
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {osceMeds.map((m) => (
            <div 
              key={m}
              onClick={() => setSelectedMed(m)}
              className="bg-neutral-900/40 border-2 border-neutral-800 p-8 rounded-[2.5rem] cursor-pointer hover:bg-neutral-900 hover:border-rose-500 transition-all group flex flex-col justify-between h-[200px] shadow-2xl relative overflow-hidden"
            >
              <div className="relative z-10">
                <span className="text-[9px] font-black text-rose-500 uppercase tracking-widest mb-2 block">Simulação Prática</span>
                <h3 className="text-4xl font-black text-white tracking-tighter italic">Med {m}</h3>
              </div>
              <div className="relative z-10 flex items-center justify-between">
                <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-widest group-hover:text-white transition-colors">Estações de Exame →</span>
